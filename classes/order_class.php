...
class Order_Class
{
	...
	public function getOrderShow($order_id,$user_id = 0,$seller_id = 0)
	{
		...
		$tb_order = new IModel('order');
 		...
 		if($data)
 		{
	    	...
	    	$tb_delivery_doc->fields = 'dd.note,dd.id,dd.delivery_code,fc.freight_name';
	    	...
	    	if($delivery_info)
	    	{
    			...
    			$data['freight']['note'] = join(",",$temp['note']);
	    	}
	...
	public function insertOrderGoods($order_id,$goodsResult = array())
	{
		...
		if(isset($goodsResult['goodsList']) && $goodsResult['goodsList'])
		{
			...
			foreach($goodsResult['goodsList'] as $key => $val)
			{
				...
				$goodsArray['cost_price'] = $val['cost_price']*$val['count'];
	 ...
	 */
	public static function sendDeliveryGoods($order_id,$order_goods_relation,$sendor = 'admin',$is_all_send=0)
	{
	...
	public static function goodsSendStatus($orderGoodsRow)
	{
	    ...
	    $data = $isDelivery ? ['未发货','已发货','已退货','部分发货'] : ['待付款','已付款','已退款'];
	...
	public static function refund($refundId,$authorId,$type = 'admin',$way = 'balance')
	{
		...
		else
		{
			...
			if($orderRow['use_partner'] > 0){
				//权益金 >= 退款金额     退款0 权益金 = 退款金额
				if($orderRow['use_partner'] >= $amount){
					$orderRow['use_partner'] = $amount;
					$amount = 0;
				}else{
					//权益金 < 退款金额     退款金额 = 原退款金额  - 全部权益金  
					$amount = $amount - $orderRow['use_partner'];
				}
			}

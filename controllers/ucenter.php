			...
			case "cancel": {
					...
					if ($model->update("id = " . $id . " and distribution_status = 0 and status = 1 and user_id = " . $this->user['user_id'])) {
						...
						if ($order_info['use_partner'] > 0) {
							$partner_account = json_decode($order_info['partner_account_json'],true);

							foreach($partner_account as $k=>$v){
								//检测账号
								$query = new IQuery("partner_account as a");
								$query->join   = 'left join partner as b on a.appid = b.appid';
								$query->where  = 'a.user_id="' . $this->user['user_id'] . '" and b.appid="' . $v['appid'] . '" and a.account_type="' . $order_info['account_type'] . '" for update';
								...
								$flag = $tb_partner_account->update("user_id = " . $user_info['user_id'] . " and appid='" . $v['appid'] . "' and account_type='" . $order_info['account_type'] . "'");
								if (!$flag) {
								...
								$insertData = array(
									...
									'event'     => '7',
									'note'      => "取消订单[{$order_info['order_no']}],回退权益金{$v['balance']}元",
									'amount'    => $v['balance'],
									'amount_log' => $amount,
									'type'      => '0',

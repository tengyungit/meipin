...
return array(
    ...
    'getSysimg' => array(
        'query' => array(
            'name'  => 'sysimg',
            'where' => 'code = "#sysimg_code#"',
            'type'  => 'row',
        )
    ),

    //品牌分类
    'getPlantformAllList' => array(
        'query' => array(
            'name'  => 'plantform',
            'order' => 'id desc',
        )
    ...
    //用户可转让权益金列表
    'getPartnerCanChange' => array(
        'file' => 'partner.php', 'class' => 'APIPartner'
    ),

    //所有权益金类型配置列表
    'getPartnerConfigALL' => array(
        'file' => 'partner.php', 'class' => 'APIPartner'
    ),

    //权益金类型配置列表
    'getPartnerConfig' => array(
        'file' => 'partner.php', 'class' => 'APIPartner'
    ),

    //平台用户资金统计
    'getAccoutCount' => array(
        'file' => 'partner.php', 'class' => 'APIPartner'
    ),
    ...
    //平台资金记录
    'getPartnerAccoutLog' => array(
        'file' => 'partner.php', 'class' => 'APIPartner'
    ),

    //平台资金数据
    'getPartnerAccount' => array(
        'file' => 'partner.php', 'class' => 'APIPartner'
    ),
    ...
    'getGoodsInfo' => array(
        'query' => array(
            ...
            'fields' => 'go.cost_price,go.name,go.id as goods_id,go.img,go.sell_price,go.point,go.weight,go.store_nums,go.exp,go.goods_no,0 as product_id,go.seller_id,go.is_delivery_fee,go.is_del,go.active_id,go.promo,go.type',
            'type'   => 'row',
        )
    ),
    ...
    'getProductInfo' => array(
        'query' => array(
            ...
            'fields' => 'go.cost_price,pro.sell_price,pro.weight,pro.id as product_id,pro.spec_array,pro.goods_id,pro.store_nums,pro.products_no as goods_no,go.name,go.point,go.exp,go.img,go.seller_id,go.is_delivery_fee,go.is_del,go.active_id,go.promo,go.type',
            'type'   => 'row',
        )
    ),
    ...
    'getPromotionList' => array(
        'query' => array(
            ...
            'fields' => 'go.market_price,p.end_time,go.img as img,p.name as name,p.award_value as award_value,go.id as goods_id,p.id as p_id,p.start_time',
    ...
    'getCommendRecom' => array(
        'query' => array(
            ...
            'fields' => 'go.store_nums,go.sale,go.virtual_sale,go.img,go.sell_price,go.name,go.id,go.market_price',
            'limit' => '10',
            'order' => 'sort asc'
        )
    ),
    ...
    'getCategoryExtendList' => array(
        'query' => array(
            ...
            'order' => 'go.sort asc',
            'fields' => 'go.sale,go.virtual_sale,go.id,go.name,go.img,go.sell_price,go.market_price',
            'limit' => 10,
        )
    ),
    ...
    'getArticleListByCatidLimit' => array(
        'file' => 'article.php', 'class' => 'APIArticle'
    ),
    ...
    'getListByCollection' => array(
        'query' => array(
            ...
            'fields' => 'o.use_partner,o.appid,o.partner_name,o.order_no,c.amount,u.username,p.name,c.id,c.pay_status,c.time',
            'where' => 'c.if_del = 0',
            'order' => 'c.id desc',
            'page'  => IReq::get('page') ? IReq::get('page') : 1,
        )
    ...
    'orderWaitCount' => array(
        'query' => array(
            'name'  => 'order',
            'fields' => 'count(id) as countNums',
            'where' => 'distribution_status = 0 and pay_status = 1 and if_del = 0 and status in (1,2)',
            'type'  => 'countNums'
        )
    ),
    ...
    //获取所有合作方列表
    'getAllPartnerList' => array(
        'query' => array(
            'name'  => 'partner',
            'fields' => 'id,partner_name,status,contact,phone,appid,appsecret,add_time,update_time',
            'where' => 'is_del = 0',
            'order' => 'id desc',
            'page'  => IReq::get('page') ? IReq::get('page') : 1,
        )

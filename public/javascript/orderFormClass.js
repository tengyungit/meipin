...
function orderFormClass()
{
	...
	this.doAccount = function()
	{
		...
		if ($('[name="appids"]:checked').val()) {
			var appIdval = $('[name="appids"]:checked').attr('atter-id');

			var bl =  $('[name="appids"]:checked').attr('attr-bl')
			var qyjprice3 = $('[name="appids"]:checked').val()
			var qyjprice2 = $('#goodsSum').text()

			$('[name="appid"]').val(appIdval)
			
			var qyjPrice1 = Number(qyjprice2) * Number(bl)/100

			if( Number(qyjprice3)  <= qyjPrice1){
				this.qyjPrice = Number(qyjprice3)
			}else{
				this.qyjPrice = qyjPrice1
			}

			if (Number(this.qyjPrice) >= Number(this.orderAmount)) {
				this.oldPrice = Number(this.orderAmount)
				this.orderAmount = 0;
				this.newqyjPrice = Number(this.qyjPrice) - Number(this.oldPrice)
				// $('[name="appids"]:checked').val(this.newqyjPrice)
				// $('[name="appids"]:checked').next().text(this.newqyjPrice)
			} else {
				this.oldPrice = Number(this.orderAmount)
				this.orderAmount = Number(this.oldPrice) - Number(this.qyjPrice)
				// $('[name="appids"]:checked').val(0)
				// $('[name="appids"]:checked').next().text(0)
			}
		}
		...
		$('#qyj_price_value').html(this.qyjPrice)
	}
	 ...
	 * 
	 */
	this.changeqyprice = function()
	{
		console.info($('[name="appids"]').length)
		// for (var i = 0; i < $('[name="appids"]').length; i++){
			// console.info()
			// $('[name="appids"]').eq(i).next().text($('[name="appids"]').eq(i).attr('attr-oldprice'))
			// if () {
...
$(function()
{
	...
	var appIds = $('[name="appids"]').first().val();
	var appId = $('[name="appids"]').first().attr('atter-id');
	console.info(appId)
	if (appIds) {
		$('[name="appid"]').val(appId)
		orderFormInstance.qyjPrice = appIds
		_self.doAccount();
	}

    ...
    function cart3()
    {
		...
		foreach($orderData as $seller_id => $goodsResult)
		{
            ...
            if(!empty($appid)){
                ...
                $query->where  = 'a.user_id="' . $user_id . '" and a.id="'.$appid.'" for update';
                $query->fields = 'a.balance,a.user_id,b.partner_name,b.appid,a.account_type';
                $array_user = $query->find();
                if (empty($array_user)) {
                    IError::show(403,'用户资金信息不存在');
                }

				$partner_account= $array_user[0];
				
				//权益金类型抵扣比例
				$partner_account_config =  new IModel("partner_account_config");
				$config = $partner_account_config->getObj("code='".$partner_account['account_type']."'");
				if(empty($config)){
					IError::show(403,'权益金抵扣比例不存在');
				}

				//最高使用权益金
				$partner_account['balance'] = $dataArray['order_amount'] * $config['percent']/100;
            ...
            if(!empty($partner_account) && $partner_account['balance'] > 0){

                if($dataArray['order_amount'] > 0 && $partner_account['balance'] >= $dataArray['order_amount']){
                    $dataArray['use_partner'] = $dataArray['order_amount'];
                    $dataArray['order_amount'] = 0;
                }else if($dataArray['order_amount'] > 0 && $partner_account['balance'] < $dataArray['order_amount']){
                    $dataArray['order_amount'] = bcsub($dataArray['order_amount'],$partner_account['balance'],2);
                    $dataArray['use_partner'] = $partner_account['balance'];
                }

                $dataArray['appid'] = $partner_account['appid'];
				$dataArray['partner_name'] = $partner_account['partner_name'];
				$dataArray['account_type'] = $partner_account['account_type'];

                //权益金日志
                if($dataArray['use_partner'] > 0){
                      //资金减少
                      $amount = $partner_account['balance'] - $dataArray['use_partner'];
                      $tb_partner_account = new IModel("partner_account");
                      $tb_partner_account->setData(array("balance" => $amount));
                      $flag = $tb_partner_account->update("user_id = " . $user_id." and id='".$appid."'");
                      if (!$flag) {
                      ...
                      $insertData = array(
                          ...
                          'user_id'   => $user_id,
                          'event'     => '3',
                          'note'      => "用户[{$this->user['username']}]使用权益金支付购买，订单[{$dataArray['order_no']}]，金额：{$dataArray['use_partner']}元",
                          'amount'    => -$dataArray['use_partner'],
                          'amount_log' => $amount,
                          'type'      => '1',
                          'time'      => ITime::getDateTime(),
						  'appid'     =>  $partner_account['appid'],

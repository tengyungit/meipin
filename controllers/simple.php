...
class Simple extends IController
{
	...
	function cart2()
	{
		...
		//渲染页面
		$this->setRenderData(['addr' => $addressList]);
		$this->redirect('cart2');
	}
	...
	function cart3()
	{
		...
		foreach ($orderData as $seller_id => $goodsResult) {
			...
			$dataArray = array(
				...
				//成本总价
				'order_cost'        => $goodsResult['orderCostPrice'],
			...
			if (!empty($appid)) {
				//权益金类型抵扣比例
				$partner_account_config =  new IModel("partner_account_config");
				$config = $partner_account_config->getObj("code='" . $appid . "'");
				if (empty($config)) {
					IError::show(403, '权益金抵扣比例不存在');
				}

				//最高使用权益金
				$canuse_max_partner_account = $dataArray['order_amount'] * $config['percent'] / 100;

				//总权益金
				$query = new IQuery("partner_account");
				$query->where  = 'user_id="' . $user_id . '" and account_type="' . $appid . '"';
				$query->fields = 'sum(balance) as total';
				$sum_partner = $query->find();
				
				//对比现有资金和最大可使用金额 如果 现有比最大使用小  最大使用就是 最大资金
				if(!empty($sum_partner) && $sum_partner['0']['total'] <= $canuse_max_partner_account){
					$canuse_max_partner_account = $sum_partner[0]['total'];
				}

				//资金账户
				$query = new IQuery("partner_account as a");
				$query->join   = 'left join partner as b on a.appid = b.appid';
				$query->where  = 'a.user_id="' . $user_id . '" and a.account_type="' . $appid . '"';
				$query->fields = 'a.id,a.balance,a.user_id,b.partner_name,b.appid,a.account_type';
				$query->order = 'a.balance desc for update';
				$partner_account_data = $query->find();
				if (empty($partner_account_data)) {
					IError::show(403, '用户资金信息不存在');
				}
			} else {
				$partner_account_data = array();
			}

			//权益金使用
			$dataArray['use_partner'] = 0;
			if (!empty($partner_account_data)) {

				if ($dataArray['order_amount'] > 0 && $canuse_max_partner_account >= $dataArray['order_amount']) {
					$dataArray['use_partner'] = $dataArray['order_amount'];
					$dataArray['order_amount'] = 0;
				} else if ($dataArray['order_amount'] > 0 && $canuse_max_partner_account < $dataArray['order_amount']) {
					$dataArray['order_amount'] = bcsub($dataArray['order_amount'], $canuse_max_partner_account, 2);
					$dataArray['use_partner'] = $canuse_max_partner_account;
				}

				//权益金日志
				if ($dataArray['use_partner'] > 0) {
					$use_total = $dataArray['use_partner'];//总使用
					$appids_str ='';//appid 逗号链接
					$names_str = '';//partner_name 逗号链接
					$partner_account_array = array();

					//处理账户资金用法
					foreach($partner_account_data as $k => $v){
						if($use_total > 0){
							if($v['balance'] >= $use_total){
								$amount = $v['balance'] - $use_total;
								$use = $use_total;
							}else{
								$amount = 0;
								$use = $v['balance'];
							}
							$use_total -= $v['balance'];//剩余
							$appids_str .= ",".$v['appid'];
							$names_str .= ",".$v['partner_name'];
							$partner_account_array[$k]['id'] =$v['id'];
							$partner_account_array[$k]['appid'] =$v['appid'];
							$partner_account_array[$k]['balance'] =$use;
	
							//资金减少
							$tb_partner_account = new IModel("partner_account");
							$tb_partner_account->setData(array("balance" => $amount));
							$flag = $tb_partner_account->update("user_id = " . $user_id . " and id='" . $v['id'] . "'");
							if (!$flag) {
							...
							$insertData = array(
								...
								'user_id'   => $user_id,
								'event'     => '3',
								'note'      => "用户[{$this->user['username']}]使用权益金支付购买，订单[{$dataArray['order_no']}]，金额：{$use}元",
								'amount'    => -$use,
								'amount_log' => $amount,
								'type'      => '1',
							...
							if (!$flag) {
								...
								IError::show(403, '用户金桥梁权益金余额更新失败');
							}
						}
					}

					//插入订单数据
					$dataArray['appid'] = trim($appids_str,",");
					$dataArray['partner_name'] = trim($names_str,",");
					$dataArray['partner_account_json'] = json_encode($partner_account_array);
					$dataArray['account_type'] = $appid;
				}
			}

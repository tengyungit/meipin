...
class Ucenter extends IController implements userAuthorization
{
	...
	public function order_status()
	{
		...
		switch($op)
		{
			case "cancel":
			{
				...
				{
                    //回退权益金
                    if($order_info['use_partner'] > 0){
                         //检测账号
                        $query = new IQuery("partner_account as a");
                        $query->join   = 'left join partner as b on a.appid = b.appid';
                        $query->where  = 'a.user_id="' . $this->user['user_id'] . '" and b.appid="'.$order_info['appid'].'" and a.account_type="'.$order_info['account_type'].'" for update';
                        ...
                        $amount = $user_info['balance'] + $order_info['use_partner'];

                        //资金增加
                        $tb_partner_account = new IModel("partner_account");
                        $tb_partner_account->setData(array("balance" => $amount));
                        $flag = $tb_partner_account->update("user_id = " . $user_info['user_id']." and appid='".$order_info['appid']."' and account_type='".$order_info['account_type']."'");
                        if (!$flag) {
                            $tb_partner_account->rollback();
                            $this->return_msg("data_error", '取消订单失败');
                        }
                        ...
                        $insertData = array(
                            ...
                            'event'     => '7',
                            'note'      => "取消订单[{$order_info['order_no']}],回退权益金{$order_info['use_partner']}元",
                            'amount'    => $order_info['use_partner'],
                            'amount_log' => $amount,
                            'type'      => '0',
    ...
    function partner_account_log()
    {
        ...
        $this->redirect('partner_account_log');
    }
